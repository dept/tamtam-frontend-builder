...
const configureNunjucksPlugin = () => {
  ...
  const contextData = {}
  const jsonData = mergeJSONData(config.data, generateFileGlobs(config.data, ['*.json', '**/*.json']))
  ...
  const templatesList = getFileList(generateFileGlobs(config.html, [
    '{*.html, !(_dev|generic|layouts)**/*.html}',
  ]))

  const pagesList = templatesList.map(template => template.replace(`${config.html}/`, ''))
  const svgList = getFileList(generateFileGlobs(config.svg, ['*.svg', '**/*.svg']), config.svg, true)
  const hasCriticalCSS = fs.existsSync(
    resolveApp(`${config.css}/critical.css`),
  )
