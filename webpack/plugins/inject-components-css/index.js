...
const createComponentsArray = folder => {
  ...
  return components.reduce((data, component) => {
    ...
    filteredFiles.forEach(file => data.push(path.join(component, file).replace(config.source, '')))
...
class InjectComponentsCSSPlugin {
  ...
  apply(compiler) {
    ...
    const compilationHandler = (compilation, callback) => {
      ...
      const componentsCssList = createComponentsArray(this.options.componentsPath).map(c => `@import '@${c.replace('.scss', '')}';`)
      const sourceFileList = getList(this.options.sourcePath)

      const existingComponentsPromises = sourceFileList.map(path =>
        new Promise((resolve, reject) => {
          fs.readFile(path, 'utf8', (err, data) => {
            if(err) {
              reject(err);
            } else {
              const regex = new RegExp(`${replaceTags(this.options.start)}(.*?)${replaceTags(this.options.end)}`, 'gsm')
              const match = data.toString().match(regex);
              if (!match) return resolve({
                path,
                data: null
              })
              const existingData = match.toString().replace(this.options.start, '').replace(this.options.end, '').split('\n').filter(d => d.length)
      ...
      Promise.all(existingComponentsPromises)
        .then(data => {
          ...
          data.filter(d => Boolean(d.data)).map(entry => {
            const componentsToBeInserted = componentsCssList.filter(component => !entry.data.includes(component))
            const insert = [...entry.data, ...componentsToBeInserted].join('\n')
            if(componentsToBeInserted.length) {
              insertPromises.push(
                replace({
                  files: entry.path,
                  from: new RegExp(`${replaceTags(this.options.start)}(.*?)${replaceTags(this.options.end)}`, 'gsm'),
