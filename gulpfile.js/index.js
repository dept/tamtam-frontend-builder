...
config.copy = function() {
  const overrideCopy =
    require(`${config.projectDirectory}/build-config/override-copy.js`) || [];
  const copyConfig = [
    {
...
config.libs = function() {
  const overrideLibs =
    require(`${config.projectDirectory}/build-config/override-libs.js`) || [];
...
function registerMainTasks(gulp) {
  ...
  gulp.task('server', gulp.series('build', 'browser-sync'));
