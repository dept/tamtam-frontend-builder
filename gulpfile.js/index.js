...
const getTask = require('./src/gulp/get-task')

const { clean } = getTask('clean')
const { copy } = getTask('copy')
const { images } = getTask('images')
const { svg } = getTask('svg')
const { injectComponentCss } = getTask('inject-component-css')
const { css } = getTask('css')
const { cssLint } = getTask('css-lint')
const { html } = getTask('html')
const { libs } = getTask('libs')
const { js } = getTask('js')
const { sw } = getTask('sw')
const { createHashes } = getTask('create-hashes')
const { updateHtmlReferences } = getTask('update-html-references')
const { browserSync } = getTask('browser-sync')
