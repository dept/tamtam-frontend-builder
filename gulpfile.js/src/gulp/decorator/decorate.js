...
function decorateTask(gulp) {
  ...
  gulp.task = function() {
    if (config.gulp.debug)
      log.debug({ sender: 'gulp/decorator', message: 'gulp.task()', data: [arguments] })
    ...
    if (config.gulp.debug) {
      log.debug({
        ...
        message:
          'Modifying gulp task ( ' +
          log.colors.cyan(arguments[0]) +
          ' ) for for better error handling...',
      })
    }
...
function decorateStart(gulp) {
  if (config.gulp.debug)
    log.debug({ sender: 'gulp/decorator', message: 'Modifying gulp for lazy loading...' })

  var gulpStartFunction = gulp.start

  gulp.start = function() {
    if (config.gulp.debug)
      log.debug({ sender: 'gulp/decorator', message: 'gulp.start()', data: [arguments] })

    for (var i = 0, leni = arguments.length; i < leni; i++) {
      if (typeof arguments[i] !== 'string') {
        // this is probably not an error but just a given callback ('function')
        if (config.gulp.verbose)
          log.warn({ sender: 'gulp/decorator', message: 'Can not load a function.' })
        continue
      }
